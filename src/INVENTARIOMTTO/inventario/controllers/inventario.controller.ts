...
import { HistorialStockService } from 'src/INVENTARIOMTTO/historial-stock/services/historial-stock.service';
import { HistorialStockDTO } from 'src/INVENTARIOMTTO/historial-stock/dto/historialStock.dto';
...
export class InventarioController {
  ...
  async updateInventoryHistoryStock(
    ...
    const acciones = await this.prismaService.iNVAccionesStock.findMany();
    ...
    const dataHistory = {
      ...
      fecha: new Date(),
    } as HistorialStockDTO;

    const history =
      await this.historialStockServices.createStockHistory(dataHistory);
